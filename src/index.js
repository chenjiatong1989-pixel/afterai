...
export async function run(argv) {
  ...
  if (options.demo) {
    options.paths = [path.resolve(dirname, "../examples")];
  }
...
export function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "privacy") options.mode = "privacy";
      ...
      options.paths.push(path.resolve(value));
      index += 1;
    } else if (arg === "--html") {
      const next = argv[index + 1];
      if (next && !next.startsWith("--")) {
        options.html = path.resolve(next);
        index += 1;
      } else {
        options.html = path.resolve("afterai-report.html");
      }
    } else {
